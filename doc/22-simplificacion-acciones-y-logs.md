...
3. **Optimización de la API (Hono)**: Inyección de base de datos a través de middleware de contexto y un manejador global de errores (`app.onError`) con clases de error de negocio tipificadas (`AppError`). Esto remueve la necesidad de bloques `try-catch` en cada endpoint.
   - Recientemente se actualizó la clase `AppError` para que su constructor reciba un objeto con atributos nombrados (`AppErrorParams: { message, statusCode, code, details, silent }`) en lugar de argumentos posicionales. Esto mejora enormemente la legibilidad y mantenibilidad del código al instanciar errores.
   - Se configuraron los errores de flujo de negocio esperado (como `DuplicateEmailError`) con la bandera `silent: true`. Esto evita que el logger emita advertencias (`logger.warn`) para comportamientos normales del usuario, reduciendo significativamente el ruido en los logs del servidor.
...
Esta clase y las clases que heredan de ella se encuentran modularizadas en `apps/api/src/errors`. (Ver la sección 8 para más detalles sobre su organización y cómo el constructor utiliza objetos `AppErrorParams`).
...
app.onError((error, c) => {
  ...
  if (error instanceof AppError) {
    logger.warn(
      `Application Error: ${error.message} (Code: ${error.code})`,
      logContext,
      error.details,
    );
    return c.json(
...
├── duplicate-email-error.ts  # 400 EMAIL_ALREADY_REGISTERED
...
### 8.5. Error de Email Duplicado (`duplicate-email-error.ts`)
...
### 8.6. Barrel de Exportación (`index.ts`)
...
### 8.7. Cómo Agregar un Nuevo Error en el Futuro
